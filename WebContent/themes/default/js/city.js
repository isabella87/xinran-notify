		   ...
		   { 
			   if(dsy.exists(str)){ 
				   ...
				   for(var i=0;i<ar.length;i++)options[length]=new Option(ar[i],ar[i]); 
				    ...
				    //if(v) options[1].setAttribute("selected", "selected");
			   } 
		   } 
  ...
  dsy.add("0_1",["东城区","西城区","崇文区","宣武区","朝阳区","丰台区","石景山区","海淀区","门头沟区","房山区","通州区","顺义区","昌平区","大兴区","怀柔区","平谷区","密云县","延庆县"]); 
   //福建
  ...
  dsy.add("0_2_8",["长泰县","东山县","华安县","龙海市","南靖县","平和县","云霄县","漳浦县","漳州市","诏安县"]); 
   //甘肃
  dsy.add("0_3",["白银","定西","甘南藏族自治州","嘉峪关","金昌","酒泉","兰州","宁夏回族自治州","陇南","平凉","庆阳","天水","武威","张掖"]); 
 ...
 dsy.add("0_10_3",["鹤岗市","萝北县","绥滨县"]); 
  ...
  ////dsy.add("0_23",["上海"]); 
  ////dsy.add("0_23_0",["黄浦区","卢湾区","徐汇区","长宁区","静安区","普陀区","闸北区","虹口区","杨浦区 ","宝山区","闵行区","嘉定区","浦东新区","金山区","松江区","青浦区","奉贤区","崇明县"]); 
  dsy.add("0_23",["黄浦区","卢湾区","徐汇区","长宁区","静安区","普陀区","闸北区","虹口区","杨浦区 ","宝山区","闵行区","嘉定区","浦东新区","金山区","松江区","青浦区","奉贤区","崇明县"]); 
   //四川
  ...
  ////dsy.add("0_25",["天津"]); 
  ////dsy.add("0_25_0",["和平区","河东区","河西区","南开区","河北区","红桥区","塘沽区","汉沽区","大港区","东丽区","西青区","津南区","北辰区","武清区","宝坻区","宁河县","静海县","蓟　县"]); 
  dsy.add("0_25",["和平区","河东区","河西区","南开区","河北区","红桥区","塘沽区","汉沽区","大港区","东丽区","西青区","津南区","北辰区","武清区","宝坻区","宁河县","静海县","蓟　县"]); 
   //西藏
  ...
  ////dsy.add("0_30",["重庆"]); 
  ////dsy.add("0_30_0",["渝中区","大渡口","江北区","沙坪坝","九龙坡","南岸区","北碚区","万盛区","双桥区","渝北区","巴南区","万州区","涪陵区","黔江区","长寿区","江津区","合川区","永川区","南川区","綦江县","潼南县","铜梁县","大足县","荣昌县","璧山县","垫江县"]); 
  dsy.add("0_30",["渝中区","大渡口","江北区","沙坪坝","九龙坡","南岸区","北碚区","万盛区","双桥区","渝北区","巴南区","万州区","涪陵区","黔江区","长寿区","江津区","合川区","永川区","南川区","綦江县","潼南县","铜梁县","大足县","荣昌县","璧山县","垫江县"]); 
   
  ////var s=["idcardAddressProv","idcardAddressCity","county"]; 
  ////var opt0 = ["省份","城市","地区"]; 三级联动也可以
  var s=["idcardAddressProv","idcardAddressCity"]; 
  var opt0 = ["省份","城市"]; 
  function setup(newS) {
	  s = newS||s;
	  ...
	  for(var i=0;i<s.length;i++) {
		  document.getElementById(s[i]).options[0].value="";
	  }
  } 
 
  function assignDsy(prov, city) {
	//修改时省市调用的方法;循环省份 、城市下拉框选项数组，若与DB中的值一样设为选中状态
	  var select_obj_prov = document.getElementsByName("idcardAddressProv")[0];
	  var select_obj_city = document.getElementsByName("idcardAddressCity")[0];
	  //循环省份下拉框
	  for(var i=0;i<select_obj_prov.length;i++){
		  var provTemp = select_obj_prov.options[i].value;
		  if(provTemp === prov){
			  select_obj_prov.options[i].selected = true;
		  }
	  }
	  //调用change(1)把城市下拉框的值再取一遍,因为第一次获取后city里面只有“城市”一个option
	  change(1);
	  //循环城市下拉框
	  for(var j=0;j<select_obj_city.length;j++){
		  var cityTemp = select_obj_city.options[j].value;
		  if(cityTemp === city){
			  select_obj_city.options[j].selected = true;
		  }
	  }
